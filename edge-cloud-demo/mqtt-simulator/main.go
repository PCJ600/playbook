...
func (d *DeviceSimulator) Connect(logger *log.Logger) error {
...
// Run 启动设备模拟
func (d *DeviceSimulator) Run(logger *log.Logger, wg *sync.WaitGroup) {
	defer wg.Done()

	retryCount := 0
	for {
		if err := d.Connect(logger); err != nil {
			if retryCount >= MAX_RETRIES {
				logger.Printf("设备 %s 达到最大重试次数，放弃连接", d.deviceID)
				return
			}
			retryCount++
			delay := time.Duration(retryCount*retryCount) * time.Second
			logger.Printf("设备 %s 连接失败，%v后重试... 错误: %v", d.deviceID, delay, err)
			time.Sleep(delay)
			continue
		}
		break
	...
	for {
		...
		if token.Wait() && token.Error() != nil {
			logger.Printf("设备 %s 发布数据失败: %v", d.deviceID, token.Error())
			continue
		}
		logger.Printf("设备 %s 发布数据: %s", d.deviceID, string(payload))
...
func main() {
	...
	logger := initLogger()
	logger.Printf("启动 %d 台设备模拟器...", SIMULATED_DEVICES)
	...
	logger.Println("收到中断信号，关闭模拟器...")
	logger.Println("所有设备已停止")
}
